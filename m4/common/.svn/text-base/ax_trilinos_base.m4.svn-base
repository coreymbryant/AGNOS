...
#   MINIMUM-VERSION be found.  If not specified, the default minimum
#   version is 8.0.0.
#
#   Supports separately specifying --with-trilinos-include or
#   --with-trilinos-libdir to override default locations underneath
#   either --with-trilinos or $TRILINOS_HOME.
#
#   On success, adds -Ipath to CPPFLAGS, -Lpath to LDFLAGS, 
#   sets the variable TRILINOS_INCLUDE based on the discovered location
#   of Trilinos_version.h, and #defines HAVE_TRILINOS.  When
...
elif test "x${TRILINOS_DIR}" != "x"; then
    trilinos_include="${TRILINOS_DIR}/include"
    trilinos_libdir="${TRILINOS_DIR}/lib"
else
  if test "x${TRILINOS_INC}" != "x"; then
    trilinos_include="${TRILINOS_INC}"
  fi
  if test "x${TRILINOS_LIB}" != "x"; then
    trilinos_libdir="${TRILINOS_LIB}"
  fi
fi
])
